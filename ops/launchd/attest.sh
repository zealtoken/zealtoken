...
if [[ -z "$ATTESTOR_PASS" ]]; then echo "$(date -u +%FT%TZ) keychain item zeal-attestor unavailable (locked or missing)" >&2; ./launchd/notify.sh "zZEC attest" "keychain item unavailable; attestation skipped"; exit 1; fi
export ATTESTOR_PASS
echo "$(date -u +%FT%TZ) attest run" >> launchd/attest.log
npm run --silent attest >> launchd/attest.log 2>&1 || { echo "$(date -u +%FT%TZ) attest FAILED; see launchd/attest.log" >&2; ./launchd/notify.sh "zZEC attest FAILED" "$(tail -1 launchd/attest.log | cut -c1-180)"; exit 1; }
