...
contract TestSwapRouter {
    ...
    function swapExactIn(PoolKey calldata key, bool zeroForOne, uint256 amountIn, uint160 limit) external returns (int256 delta) {
        bytes memory r = pm.unlock(abi.encode(msg.sender, key, zeroForOne, amountIn, limit));
        delta = abi.decode(r, (int256));
    }
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        require(msg.sender == address(pm), "pm");
        (address user, PoolKey memory key, bool zeroForOne, uint256 amountIn, uint160 limit) = abi.decode(data, (address, PoolKey, bool, uint256, uint160));
        int256 d = pm.swap(key, SwapParams(zeroForOne, -int256(amountIn), limit), "");
