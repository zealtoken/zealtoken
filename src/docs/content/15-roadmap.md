...
A launchpad for tokens paired with zZEC instead of ETH. Built and tested, deploying on a date still to be set. One transaction puts a token's whole supply into a locked Uniswap v4 pool; the creator holds nothing and picks the fee (1% to 5%) and where it goes: at least 0.5% buys back and burns $ZEAL, at most 0.5% to the creator, 0.5% to the platform, the rest paid to holders as zZEC dividends every day. Creators choose a ten minute batch opening or an instant one, and buyers can pay with ETH routed through zZEC in one click. Contracts pass unit tests and a full lifecycle on a fork of the live chain. [zealz.fun](https://zealz.fun) shows the interface with sample launches. The full design is on [zealz.fun, the launchpad](#/launchpad).
