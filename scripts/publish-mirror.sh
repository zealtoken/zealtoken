...
# committer becomes the project identity, `.claude/` is dropped from every
...
ARGS=(--force --path .claude --invert-paths --mailmap "$TMP/mailmap" --replace-message "$TMP/msgmap")
...
if git log --all --name-only --format= | grep -q '^\.claude/'; then echo ".claude still in history"; exit 3; fi
